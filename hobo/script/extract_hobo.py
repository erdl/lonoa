...
def get_csv_from_folder_not_in_db(conn, csv_filename):
    ...
    for sensor_info_row in sensor_info_rows:
        error_log_row = orm_hobo.ErrorLog(status=True, purpose_id=sensor_info_row.purpose_id, datetime=current_time, pipeline_stage=orm_hobo.PipelineStage.data_acquisition)
...
def insert_csv_readings_into_db(conn, csv_readings, csv_metadata, csv_filename):
    ...
    for sensor_info_row in sensor_info_rows:
        ...
        error_log_row = orm_hobo.ErrorLog(status=True, purpose_id=sensor_info_row.purpose_id, datetime=current_time, pipeline_stage=orm_hobo.PipelineStage.database_insertion)
        conn.add(error_log_row)
        # need to flush and refresh to get error_log_row.log_id
        conn.flush()
        conn.refresh(error_log_row)
...
def log_failure_to_get_csv_readings_from_folder_not_in_db(conn, csv_filename, exception):
    ...
    error_log_row = orm_hobo.ErrorLog(status=False, datetime=current_time, error_type=exception.__class__.__name__, pipeline_stage=orm_hobo.PipelineStage.data_acquisition)
    conn.add(error_log_row)
    # need to flush and refresh to get error_log_row.log_id
    conn.flush()
    conn.refresh(error_log_row)
...
def log_failure_to_insert_csv_readings_into_db(conn, csv_filename, csv_metadata, exception):
    ...
    for sensor_info_row in sensor_info_rows:
        ...
        if not new_readings:
            error_log_row = orm_hobo.ErrorLog(purpose_id=sensor_info_row.purpose_id, datetime=current_time, error_type=exception.__class__.__name__,pipeline_stage=orm_hobo.PipelineStage.database_insertion)
        # set status to False if readings were new but an error was thrown
        else:
            error_log_row = orm_hobo.ErrorLog(purpose_id=sensor_info_row.purpose_id, status=False, datetime=current_time, error_type=exception.__class__.__name__,  pipeline_stage=orm_hobo.PipelineStage.database_insertion)
        conn.add(error_log_row)
        # need to flush and refresh to get error_log_row.log_id
        conn.flush()
        conn.refresh(error_log_row)
