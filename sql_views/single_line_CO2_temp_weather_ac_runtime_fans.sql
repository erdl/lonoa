-- Grab indoor and weather conditions, fan and AC power usage,
-- time elapsed since last AC usage, and AC runtime.
-- Displays all units for the same time increment in one row 
-- [used by NA]
CREATE VIEW single_line_CO2_temp_weather_ac_runtime_fans AS 
 ...
 SELECT 'frog-1'::text AS building,
    ...
    date_part('epoch'::text, age(frog1.datetime, max(
        CASE
            WHEN frog1."AC air handling unit power kW" > 0.01::double precision THEN frog1.datetime
            ELSE NULL::timestamp without time zone
        END) OVER (ROWS BETWEEN 864 PRECEDING AND 0 PRECEDING))) / 60::double precision AS "AC AHU minutes since last on",
    date_part('epoch'::text, age(frog1.datetime, max(
        CASE
            WHEN frog1."AC air handling unit power kW" <= 0.01::double precision THEN frog1.datetime
            ELSE NULL::timestamp without time zone
        END) OVER (ROWS BETWEEN 864 PRECEDING AND 0 PRECEDING))) / 60::double precision AS "AC AHU minutes runtime"
...
UNION
 SELECT 'frog-2'::text AS building,
    ...
    date_part('epoch'::text, age(frog2.datetime, max(
        CASE
            WHEN frog2."AC air handling unit power kW" > 0.01::double precision THEN frog2.datetime
            ELSE NULL::timestamp without time zone
        END) OVER (ROWS BETWEEN 864 PRECEDING AND 0 PRECEDING))) / 60::double precision AS "AC AHU minutes since last on",
    date_part('epoch'::text, age(frog2.datetime, max(
        CASE
