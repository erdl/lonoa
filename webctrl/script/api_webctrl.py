...
def get_data_from_api(sensor, conn):
    ...
    webctrl_user_row = conn.query(orm.ApiAuthentication.username, orm.ApiAuthentication.password).filter_by(script_folder=orm.SensorInfo.ScriptFolderEnum.webctrl)[0]
    ...
    if readings.status_code == requests.codes.ok:
        ...
        error_log_row = orm.ErrorLog(datetime=current_time, was_success=True, purpose_id=sensor.purpose_id, pipeline_stage=orm.ErrorLog.PipelineStageEnum.data_acquisition)
...
def insert_readings_into_database(conn, readings, sensor):
    ...
    for reading in readings:
        ...
        if reading_time > pendulum.instance(sensor.last_updated_datetime, tz='Pacific/Honolulu') and reading_time != previous_reading_time:
            reading_row = orm.Reading(purpose_id=sensor.purpose_id, datetime=reading_time, reading=reading_value, units=sensor.unit, upload_timestamp=current_time)
    ...
    if new_last_updated_datetime:
        conn.query(orm.SensorInfo).filter(orm.SensorInfo.purpose_id == sensor.purpose_id).update(
            {"last_updated_datetime": new_last_updated_datetime})
    error_log_row = orm.ErrorLog(datetime=current_time, was_success=True, purpose_id=sensor.purpose_id, pipeline_stage=orm.ErrorLog.PipelineStageEnum.database_insertion)
    conn.add(error_log_row)
    ...
    conn.query(orm.Reading.log_id). \
        filter(orm.Reading.purpose_id == sensor.purpose_id,
...
def log_failure_to_connect_to_api(conn, exception, sensor):
    ...
    error_log_row = orm.ErrorLog(datetime=current_time, error_type=exception.__class__.__name__, pipeline_stage=orm.ErrorLog.PipelineStageEnum.data_acquisition, purpose_id=sensor.purpose_id, was_success=False)
    conn.add(error_log_row)
    conn.commit()
...
def log_failure_to_connect_to_database(conn, exception, sensor):
    ...
    error_log_row = orm.ErrorLog(datetime=current_time, error_type=exception.__class__.__name__, pipeline_stage=orm.ErrorLog.PipelineStageEnum.database_insertion, purpose_id=sensor.purpose_id, was_success=False)
    conn.add(error_log_row)
    conn.commit()
...
if __name__ == '__main__':
    ...
    conn = get_db_handler()
    sensors = conn.query(orm.SensorInfo.purpose_id, orm.SensorInfo.query_string, orm.SensorInfo.last_updated_datetime, orm.SensorInfo.unit).filter_by(script_folder=orm.SensorInfo.ScriptFolderEnum.webctrl, is_active=True)
